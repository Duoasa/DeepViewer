...
把 `dsh-plugin-subscriptions` 作为固定版本的 DSH 扩展接入 DeepViewer 开发版，使用户可在
...
- NG-003：不封包、签名、公证或发布安装资产。
- NG-004：Keychain 凭据适配是正式分发门禁，不在本开发集成切片中伪装完成。
...
- R-006：正式分发前必须完成凭据存储安全复核，并提供 macOS Keychain 适配或获批的等价方案。
...
加载边界与日志策略。开发态凭据文件不得进入 Runtime；正式分发适用 R-006 门禁。
...
| 明文长期令牌 | 本机账户风险 | 隔离目录与 0600；正式分发前执行 R-006 |
