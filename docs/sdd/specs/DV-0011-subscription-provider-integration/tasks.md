...
- [x] T-002 `[R-001, R-008, AC-005]` 固定插件依赖并纳入可复现 Runtime。
- [x] T-003 `[R-002, R-007, AC-001, AC-004]` 实现 manifest 预检、官方 patch 加载和降级诊断。
- [x] T-004 `[R-003, AC-002]` 将订阅组合进“模型”页面，并验证本地化设置与 OAuth 登录。
...
- [ ] T-008 `[R-004, AC-003]` 使用真实订阅账户复验 provider/model/tool 实际调用与登出清理。

## 延后事项

| 项目 | 原因 | 后续规格 |
| --- | --- | --- |
| macOS Keychain 凭据迁移 | `v0.2.1` 仅批准隔离 `0600` 临时方案，需要独立存储适配与迁移设计 | 稳定版前领取 |
