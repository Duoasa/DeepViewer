...
- G-005：保证所有公开版本都是从对应版本源码重新生成、且不携带开发者个人设置或凭据的全新资产。
...
- R-010：每个公开发布的 arm64/x64 安装资产必须从当前版本的明确允许输入重新构建，不得包含任何开发者/维护者个人设置、工作区内容、会话、日志、主目录元数据、API Key、Token、Authorization 值或其他凭据。
...
- NFR-008：发布流水线必须在每个架构构建前清理 Runtime、staging、应用输出和 DMG，只从 allowlist staging 封包，并在生成公开资产前执行不泄露秘密原文的自动净化审计。
...
- AC-011：Given 任一拟公开的版本与架构，When 执行发布封包，Then 旧 Runtime/staging/应用/DMG 已清理，ASAR 仅包含 allowlist 输入，最终 `.app` 不含敏感文件名、开发机/主目录绝对路径或当前构建环境的凭据值；任一命中都会在 DMG 生成或上传前失败。
...
- 公开发布不得从 Electron `userData`、开发者主目录、现有安装、历史 DMG 或历史 `.app` 复制内容；包管理器下载缓存可以复用，但最终 Runtime 与安装资产必须重新生成并通过净化审计。
...
- 公开资产以临时 allowlist staging 为唯一应用输入；构建阶段发现的开发机路径必须删除或替换为稳定的非个人占位路径，包管理器工作区元数据不得随 Runtime 分发。
...
| 发布包夹带个人配置、绝对路径或凭据 | 隐私泄露并污染公开发行 | 全新 Runtime/输出、allowlist staging、路径净化、环境凭据值扫描与上传前阻断 |
