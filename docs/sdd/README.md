...
│       └── ADR-0001-*.md         # 持久架构决策
├── integrations/
│   └── dsh-plugins.md           # 手动集成插件与内核更新检查表
├── releases/
│   ├── README.md                # 公开版本与资产索引
│   └── vX.Y.Z.md                # 发布元数据、产物和验证边界
└── specs/
    ├── README.md                # 功能规格索引
    ├── _template/               # 新规格模板
    │   ├── spec.md
...
日常迭代与文档批处理规则以 [`governance.md`](governance.md) 为唯一来源，避免在入口文档
重复维护同一套约束。
...
3. [`integrations/dsh-plugins.md`](integrations/dsh-plugins.md) 定义当前手动集成的 DSH 插件及
   内核更新检查门禁。
4. 已批准的功能规格定义单项变更的预期行为。
5. 实现和测试证明当前行为；如果与批准规格不一致，必须在同一变更中修正规格或实现。
6. `releases/` 记录已经发生的公开发布及其资产证据；预览版发布不会自动把尚未完成验收的规格提升为 `Released`。
