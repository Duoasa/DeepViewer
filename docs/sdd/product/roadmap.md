...
当前证据：`v0.1.1` 已公开交付 DeepViewer 应用身份、macOS 一体化标题栏、Codex 式侧栏与顶部安全区，以及两阶段品牌加载页面。正在准备的 [`v0.1.2`](../releases/v0.1.2.md) 候选版本进一步把窗口收束为侧栏与 Chat 两列结构，统一输入框底部基准、顶部模型统计、新会话欢迎内容、主题同步和聚焦/失焦侧栏材质，并建立受控上游 UI 覆盖与本地快速迭代工作流。维护者已完成本轮工作区 UI 的逐轮验收；正式双架构安装包与公开发布证据仍待发布门禁完成。减少动态效果和 Runtime 失败态继续保留专项人工验收项。
