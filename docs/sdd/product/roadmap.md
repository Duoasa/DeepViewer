...
当前证据：[`v0.2.1`](../releases/v0.2.1.md) 已在 `v0.1.2` 工作区体验基础上升级到
DeepSeek Harness rc.7，并交付全局设置/关于页、订阅提供方集成、代码与静态网页预览侧栏、
Agent 产出内置预览及 Finder 原生操作。arm64 与 x64 包已通过正式签名、公证、隐私和架构门禁；
真实 Intel Mac、订阅实际模型/工具调用与登出、减少动态效果和 Runtime 失败态仍保留专项验收。
...
当前证据：`v0.2.1` 已通过固定 DSH 扩展契约集成订阅与预览两个插件，并提供可禁用、可降级的
启动路径。静态代码/网页预览已经进入产品；终端输出、编辑、任意 dev-server 与更完整的插件管理
继续留在本阶段后续规格。
